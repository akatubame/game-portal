...
export function getRotationMatchRewards(
  ...
  // Prism Break is the top-priority reward. Only direct normal-color
  // matches are present in matchLines, so special-effect clears never
  // inflate this condition. When it is met, no other reward is generated.
  if (createsPrismBreak) {
    addReward(COLOR_BREAKER_BLOCK, matchLines);
    return rewards;
  }
  ...
  diagonalLines
    .filter((line) => line.cells.length >= 5)
    .forEach((line) => addReward(BOMB_BLOCK, [line]));
