...

{
  ...
  assert.deepEqual(
    ...
    "ten or more simultaneous horizontal, vertical, and diagonal clears reward only Prism Break"
  );
  ...
  assert.deepEqual(
    ...
    [{ key: "3:2", token: COLOR_BREAKER_BLOCK }],
    "ten or more direct clears across any two direction types reward only Prism Break"
  );

  const nineCellRewards = getRotationMatchRewards([
    {
      cells: ["3:0", "3:1", "3:2", "3:3", "3:4"],
      color: "coral",
      direction: "horizontal",
  ...
  assert.equal(
    nineCellRewards.some((reward) => reward.token === COLOR_BREAKER_BLOCK),
    false,
    "nine direct clears across two direction types do not reward Prism Break"
  );
}
